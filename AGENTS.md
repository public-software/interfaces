...
This is `interfaces`, one repository of [Public Software](https://publicsoftware.dev): a spec-first cleanroom Rust suite built by people and coding agents together. Read this file before the README; it is the contract every agent works under here.

**Ring:** spine · **Layers:** all · **Wave:** 1 · Every cross-repo API as WIT packages and wire schemas, with generated binding crates. Planned components: public:doc/* · ui/* · plugin/* · identity/* · store/* · media/* · net/*; pub-interfaces-* crates.
...
- A new crate is `pub new <kind> <name>` with a kind from lib, app, service, plugin, spec; it lands in `crates/pub-interfaces-<name>/` and gets a `[[component]]` entry in `CATALOG.toml`.

## Conventions

- Crates are named `pub-interfaces-<name>`; a service is the binary `pubd-<name>`; WIT interfaces live under `public:`.
...
- The merge gates decide (`pub check`, CI, the agent review `suite / policy`, the maintainer team `public-software/maint-interfaces`), not who or what wrote the change.
<!-- agents:end -->

## Repo notes

_Maintainers of `interfaces` write here what the generated section cannot know: how the crates fit together, how to run the thing, which specifications matter, what not to touch. Everything above this heading is regenerated._
